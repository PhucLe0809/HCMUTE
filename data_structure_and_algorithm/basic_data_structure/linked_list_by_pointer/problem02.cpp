...
struct IntList{
    ...
    void insert_between(IntNode *pos, IntNode *inter){
        ...
        IntNode* pointer = phead;
        while (pointer != nullptr && pointer->next != pos){
            pointer = pointer->next;
        }
        if (pointer != nullptr) {
            inter->next = pos;
    ...
    void move_between(IntNode *pos){
        ...
        IntNode* prevNode = phead;
        while (prevNode != nullptr && prevNode->next != pos){
            prevNode = prevNode->next;
        }
        if (prevNode != nullptr){
            IntNode* nodeToMove = prevNode->next;
...
struct CarList{
    ...
    void move_between(CarNode *pos){
        ...
        CarNode* prevNode = phead;
        while (prevNode != nullptr && prevNode->next != pos){
            prevNode = prevNode->next;
        }
        if (prevNode != nullptr){
            CarNode* nodeToMove = prevNode->next;
            prevNode->next = nodeToMove->next;
            nodeToMove->next = nullptr;
            cnt--;
        }
    ...
    CarNode* find_value(CarNode *car){
        CarNode *pos = phead;
        while (pos != nullptr){
            if (pos->comp_pair(car)) return pos;
            pos = pos->next;
        }
        return nullptr;
    }
